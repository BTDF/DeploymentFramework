'--------------------------------------------------------------------------
'
' WMI script to remove a send port group.
'
	...
	If objInstSet.Count > 0 then
		For Each objInst in objInstSet
			...
			If Err <> 0	Then
				PrintWMIErrorThenExit Err.Description, Err.Number
			End If
			WScript.Echo "The Send Port Group was successfully unenlisted."
			...
			If Err <> 0	Then
				PrintWMIErrorThenExit Err.Description, Err.Number
			End If
			WScript.Echo "The Send Port Group was successfully removed."
		Next
	Else
...
Sub PrintUsage()
	WScript.Echo "Usage:" + Chr(10) + Chr(10) + _
				 "cscript RemoveSendPortGroup.vbs <Send Port Group Name>" + _
				 Chr(10) + Chr(10) + "Where: " + Chr(10) + _
				 "  <Send Port Name> = The name of the send port group to remove." + _
