...
Dim comApp, errorString, originalSetting
errorString = ""
...
call comApp.SetComponentProperty(objArgs(1), objArgs(2), objArgs(3), originalSetting, errorString)
if errorString <> "" then
...
Class COMApplication
  ...
  For Each compObject In components
    If LCase(compObject.Name) = LCase(compName) Then
...
Sub PrintUsage()
	WScript.Echo "Usage:"
	WScript.Echo
	WScript.Echo "cscript SetComponentPropery.vbs <COMPlusPackageName> <ComponentProgId> <PropertyName> <PropertyValue>"
	WScript.Echo
	WScript.Echo
End Sub
